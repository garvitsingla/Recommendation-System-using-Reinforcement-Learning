...
class DRRAgent:
    ...
    def __init__(self, env, users_num, items_num, state_size, is_test=False, use_wandb=False):
        ...
        self.save_model_weight_dir = "./save_model/fixed_trail"

        # Create base model directory
        if not os.path.exists(self.save_model_weight_dir):
            os.makedirs(self.save_model_weight_dir)

        # Correctly create the 'images' subdirectory
        ...
        self.epsilon = 1.
        self.epsilon_decay = (self.epsilon - 0.1)/5000
        self.std = 1.5
    ...
    def train(self, max_episode_num, top_k=False, load_model=False):
        ...
        for episode in range(max_episode_num):
            ...
            if (episode+1)%1000 == 0 or episode == max_episode_num-1:
                self.save_model(os.path.join(self.save_model_weight_dir, 'actor_fixed.weights.h5'),
                                os.path.join(self.save_model_weight_dir, 'critic_fixed.weights.h5')
                                )
